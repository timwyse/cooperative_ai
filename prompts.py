...
def generate_trade_system_info(player):
    if not player.pay4partner:
        trading_rules = """- You are in 'regular' mode: You may trade chips with the other player at any agreed rate (e.g., 1 green for 1 blue, 1 green for 2 red, 2 green for nothing, etc.)."""
    else:
        trading_rules = """-The game is in 'pay for other player' mode. This means that trades are not made by directly swapping chips. Instead, when a trade agreement is reached, each player commits to covering the cost of the other’s movement onto the agreed color, for a limited number of times as specified in the trade". In practice:
•   If the other player moves onto a tile of a color you agreed to cover, you pay the chip for that move.
•   If you move onto a tile of a color the other player agreed to cover, they pay the chip for you.
This applies only to color and number of chips specified in the agreement. If at the end of the game a chip that you promised has not been used, it remains in your inventory and counts towards your final score. The same applies to chips promised to you by the other player.
"""
...
3. Decision:
  - If you can move toward your goal output the move in format "r,c" (e.g. "1,2")
...
Think step by step about whether to accept this trade. Consider your current chip inventory, your best path to your goal, and whether this trade helps you reach your goal more easily. Also consider whether the trade would result in having more chips left over after reaching your goal, and hence a higher score.
...
2.
{{
 "rationale": "This trade gives me 2 red chips in exchange for 1 blue chip. While I don’t need red chips to reach my goal, I have excess blue chips, and this trade gives me more total chips and hence a higher score when I reach my goal.",
 "answer": "yes"
}}
...
You have been offered a 'pay for other player' arrangement:
...
def generate_pay4partner_prompt(player, player_context, color, agreements):
    ...
    return f"""
{player_context}
Recall the 'pay for other player' mode rules:
{pay4partner_info}
...
def generate_pay4partner_mode_info(player, short_summary=False):
    if player.pay4partner:
        ...
        pay4partner_mode_info = """
Important Note: The game is in 'pay for other player' mode. This means that trades are not made by directly swapping chips. Instead, when a trade agreement is reached, each player commits to covering the cost of the other’s movement onto the agreed color, for a limited number of times as specified in the trade". In practice:
•   If the other player moves onto a tile of a color you agreed to cover, you pay the chip for that move.
•   If you move onto a tile of a color the other player agreed to cover, they pay the chip for you.
This applies only to color and number of chips specified in the agreement. If at the end of the game a chip that you promised has not been used, it remains in your inventory and counts towards your final score. The same applies to chips promised to you by the other player.
"""
...
\n- So far you have promised to cover these colors/quantities for the other player: {promised_resources_to_give if promised_resources_to_give else '{}'}"
\n- So far you have been promised to be covered for these colors/quantities by the other player: {promised_resources_to_receive if promised_resources_to_receive else '{}'}
...
Thus if you move onto one of these tiles, you do not need to have the required chip in your inventory to move onto that tile, nor do you need to trade for it. The same is true for the other player.
"""
...
Remember this when deciding your next move or proposing/accepting trades, as you may be incentivised to help the other player to finish.
"""
...
Note that although this player appears as the 'user' in your chat, they are also an AI agent similar to you.
...
When a contract is agreed upon, you will be able to access the tiles specified in the contract without needing to have the chip in your inventory, as the chip will automatically be taken from the other player's chip inventory. The same is true for the other player. Therefore it is important the contract specifies all tiles where you will need a chip.
...
When you have both agreed to a contract, a judge will summarise the contract in JSON format and present it back to you for you and the other player to agree one last time.
"""
...
Think step by step about your possible routes and the chips you will need at each specific tile along your path.
Consider also the other player's possible routes and chips they will need, and who needs help from the other player more.


You are now going to have a conversation with the other player (ie the user in the chat). You must negotiate a contract with this player whereby you specify how many of the points that you get for winning you will give to the other player if you reach your goal, and how many points that they get for winning they will give you if they reach their goal.
Note that although this player appears as the 'user' in your chat, they are also an AI agent similar to you.


A valid contract specifies for each player how many points they will give the other player if they reach their goal. The most points you can give to the other player is {POINTS_FOR_WIN}, because that is the number of points that each player gets for reaching their goal. The least points you can give is 0. You may not give negative points. You may propose, counter, or modify the terms set out by the other player.
